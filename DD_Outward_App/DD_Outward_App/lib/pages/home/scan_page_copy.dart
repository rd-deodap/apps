...
class _ScanPageState extends State<ScanPage> {
  ...
  final MobileScannerController cameraController = MobileScannerController(
    ...
    detectionSpeed: DetectionSpeed.noDuplicates, // prevent duplicates
  );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: appBar(
        ...
        actions: [
          IconButton(
            icon: const Icon(Icons.flip_camera_android, color: Colors.black54),
            onPressed: () {
  ...
  void _onBarcodeScanned(String code) {
    ...
    if (!isValidBarcode(code)) {
      playSoundError();
      toastError("Invalid barcode");
      return;
    }

    isScanning = true;
    AppConstant.SCAN_ID = code;
