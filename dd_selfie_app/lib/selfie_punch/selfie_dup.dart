...
class _SelfiePunchPageState extends State<SelfiePunchPage>
  ...
  Future<void> _loadLocationPolicy() async {
    ...
    try {
      ...
      if (js["ok"] == true && js["data"] != null) {
        ...
        if (parsedLat != null && parsedLng != null) {
          ...
          if (isInvalidLat || isInvalidLng) {
            ...
            if (kDebugMode) {
              print(
                "⚠️ Invalid coordinates from backend: lat=$latValue, lng=$lngValue",
              );
            }
          }
        }
        ...
        // Debug logging
        if (kDebugMode) {
          print("📍 Location Policy Loaded:");
          print("   Mode: $mode");
          print("   Base Lat: $parsedLat");
          print("   Base Lng: $parsedLng");
          print("   Radius: $parsedRadius meters");
        ...
        if (!mounted) return;
        if (kDebugMode) {
          print("⚠️ Location policy API returned ok=false or no data");
        }
        setState(() {
          _policyLoaded = true;
      ...
      if (!mounted) return;
      if (kDebugMode) {
        print("❌ Location policy load failed: $e");
      }
      setState(() {
        _policyLoaded = true;
        _locationMode = "EVERYONE";
  ...
  void _checkDistanceFromBase() {
    ...
    if (_locationMode == "EVERYONE" || _baseLat == null || _baseLng == null) {
      ...
      if (kDebugMode) {
        print(
          "📍 Distance check: BYPASSED (mode=$_locationMode, hasCoords=${_baseLat != null && _baseLng != null})",
        );
      }
      return;
    }
    ...
    if (_pos == null) {
      ...
      if (kDebugMode) {
        print("📍 Distance check: FAILED (current position unavailable)");
      }
      return;
    }
    ...
    if (kDebugMode) {
      ...
      print("   Current: (${_pos!.latitude}, ${_pos!.longitude})");
      print("   Base: ($_baseLat, $_baseLng)");
      print("   Distance: ${distance.toStringAsFixed(2)} meters");
      print("   Allowed: $_allowedRadius meters");
      print("   Within Range: $withinRange");
    }
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        if (kDebugMode && attempt > 1) {
          print("🔄 Webhook retry attempt $attempt/$maxRetries");
        }
        ...
        if (response.statusCode >= 500 && attempt < maxRetries) {
          if (kDebugMode) {
            print("⚠️ Server error ${response.statusCode}, retrying in ${retryDelay.inSeconds}s...");
          }
        ...
        if (kDebugMode) {
          print("⏱️ Webhook timeout on attempt $attempt: $e");
        }
        ...
        if (kDebugMode) {
          print("❌ Webhook send failed on attempt $attempt: $e");
        }
        if (attempt < maxRetries) {
          if (kDebugMode) {
            print("🔄 Retrying in ${retryDelay.inSeconds}s...");
  ...
  String _calculateLateIn(String inTime) {
    ...
    try {
      ...
      if (kDebugMode) {
        print("❌ Error calculating late in: $e");
      }
      return "00:00";
    }
  }
  ...
  String _buildAutoRemark() {
    ...
    if (inTime == "-" || outTime == "-") {
      ...
      if (lateIn != "00:00") {
        ...
        try {
          ...
          if (kDebugMode) print("❌ Error checking early in: $e");
        }
      }
    }
    ...
    if (otTime != "00:00:00" && otTime != "00:00") {
      try {
        ...
        if (kDebugMode) print("❌ Error checking overtime: $e");
      }
    }
    ...
    if (erlOut != "00:00") {
      try {
        ...
        if (kDebugMode) print("❌ Error checking early out: $e");
      }
    }
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  Text(
                    "Your location: ${_pos?.latitude.toStringAsFixed(6) ?? 'N/A'}, ${_pos?.longitude.toStringAsFixed(6) ?? 'N/A'}",
                    style: const TextStyle(fontSize: 10, color: Colors.black54),
                  ),
                  Text(
                    "Base location: ${_baseLat?.toStringAsFixed(6) ?? 'N/A'}, ${_baseLng?.toStringAsFixed(6) ?? 'N/A'}",
                    style: const TextStyle(fontSize: 10, color: Colors.black54),
                  ),
                  Text(
                    "Mode: $_locationMode | Radius: $_allowedRadius m",
                    style: const TextStyle(fontSize: 10, color: Colors.black54),
                  ),
                ],
              ),
  ...
  /// Debug info panel - only shown in debug mode
  Widget _buildDebugPanel({EdgeInsets? margin}) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: const [
              Icon(CupertinoIcons.wrench_fill, size: 14, color: Colors.yellow),
              SizedBox(width: 6),
              Text(
                "DEBUG MODE - Location Policy Status",
                style: TextStyle(
                  fontSize: 11,
                  fontWeight: FontWeight.w900,
                  color: Colors.yellow,
          ...
          const SizedBox(height: 8),
          _debugRow("Policy Loaded", _policyLoaded ? "✅ YES" : "❌ NO"),
          _debugRow("Mode", _locationMode ?? "NULL"),
          _debugRow(
            "Base Coords",
            _baseLat != null && _baseLng != null
                ? "✅ SET (${_baseLat?.toStringAsFixed(4)}, ${_baseLng?.toStringAsFixed(4)})"
                : "❌ NULL",
          ),
          _debugRow("Allowed Radius", "$_allowedRadius meters"),
          _debugRow("Base Address", _baseAddress ?? "NULL"),
          ...
          _debugRow("Within Range", _isWithinRange ? "✅ YES" : "❌ NO"),
          _debugRow("Location Service", _locServiceEnabled ? "✅ ON" : "❌ OFF"),
          _debugRow(
            "Location Permission",
            _locPermissionGranted ? "✅ GRANTED" : "❌ DENIED",
          ),
          const Divider(color: Colors.white24, height: 16),
          Text(
            _locationMode == "EVERYONE" || _baseLat == null || _baseLng == null
                ? "⚠️ LOCATION RESTRICTION: BYPASSED"
                : _isWithinRange
                ? "✅ LOCATION RESTRICTION: ALLOWED"
                : "❌ LOCATION RESTRICTION: BLOCKED",
            style: TextStyle(
              fontSize: 11,
              fontWeight: FontWeight.w900,
              color:
