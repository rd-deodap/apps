...
class NDRScreen extends StatefulWidget {
  ...
  const NDRScreen({super.key, required this.awb, this.payload});
...
class _NDRScreenState extends State<NDRScreen> {
  ...
  final List<Map<String, String>> _ndrReasons = const [
    {"value": "customer_unavailable", "label": "Customer Unavailable"},
    {"value": "customer_refused", "label": "Customer Refused"},
    {"value": "reschedule_requested", "label": "Customer Requested Reschedule"},
    {"value": "payment_issue", "label": "Payment Not Ready (COD)"},
    {"value": "address_incomplete", "label": "Incomplete Address"},
    {"value": "address_wrong", "label": "Wrong Address"},
    {"value": "address_not_found", "label": "Address Not Found"},
    {"value": "contact_incorrect", "label": "Incorrect Contact Number"},
    {"value": "not_reachable", "label": "Phone Unreachable/Switched Off"},
    {"value": "office_closed", "label": "Office/Shop Closed"},
    {"value": "out_of_area", "label": "Out of Delivery Area"},
    {"value": "access_denied", "label": "Gate/Security Access Denied"},
    {"value": "no_attempt", "label": "No Attempt to Deliver"},
    {"value": "weather_disaster", "label": "Extreme Weather/Natural Disaster"},
    {"value": "vehicle_breakdown", "label": "Courier Vehicle Breakdown"},
    {"value": "delivery_failed_general", "label": "Delivery Failed (Other)"},
    {"value": "undelivered_general", "label": "Undelivered (General)"},
  ...
  // ---------------- API: NDR ----------------
  Future<void> _submitNDR() async {
    if (_isLoading) return;
    if (!_formKey.currentState!.validate()) return;

    if (_selectedReason == null) {
      _toast("Please select an NDR reason");
      return;
    }

    final manualReason = _manualReasonController.text.trim();
    final finalReason = (_selectedReason == 'other')
        ? manualReason
        : _selectedReason!;

    if (_selectedReason == 'other' && manualReason.isEmpty) {
    ...
    try {
      ...
      _toast(
        body["message"]?.toString() ?? (ok ? "NDR submitted" : "NDR failed"),
        success: ok,
      );
  ...
  Widget _buildSummary(Map<String, dynamic> body) {
    ...
    // data could be Map OR List OR String on failure -> safe parse
    ...
    return Column(
      ...
      children: [
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        success ? "NDR Submitted" : "NDR Failed",
                        style: TextStyle(
                          fontSize: 15,
                          fontWeight: FontWeight.w900,
                          color: headerFg,
        ...
        // If order exists, show details; else skip (many errors won't have order)
        if (order.isNotEmpty) ...[
          _sectionCard(
            "Order",
            Column(
                ...
                child: Column(
                  children: [
                    DropdownButtonFormField<String>(
                      ...
                      validator: (v) =>
                          v == null ? "Please select an NDR reason" : null,
                    ),
                    ...
                    if (showManual) ...[
                      ...
                      TextFormField(
                        ...
                        decoration: InputDecoration(
                          ...
                          hintText:
                              "Example: Door closed / Customer not at home",
                    ...
                    Container(
                      ...
                      child: const Row(
                        children: [
                          Icon(Icons.info_outline, color: Color(0xFF1E63B5)),
                          SizedBox(width: 10),
                          Expanded(
                            child: Text(
                              "NDR will mark the order for retry/next attempt.",
                              style: TextStyle(
                                color: titleText,
                                fontWeight: FontWeight.w800,
                              ),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: _isLoading ? null : _submitNDR,
                        style: ElevatedButton.styleFrom(
