...
class ApiBaseHelper {
  ...
  Future<dynamic> postWithFileAndHeader(String url, Map body, String token, String filePath) async {
    debugPrint(_baseUrl + url);
    debugPrint(jsonEncode(body));
    debugPrint("token is :::: ");
    debugPrint(token);
    ...
    var request = http.MultipartRequest("POST", Uri.parse("https://aaizoon.com/" + url));
    //add text fields
    ...
    debugPrint("path--");
    debugPrint(filePath);
    var pic = await http.MultipartFile.fromPath("profile_pic", filePath, contentType: MediaType('image', 'jpeg'));
    ...
    try {
      ...
      respJson.putIfAbsent("message", () => response.statusCode == 200? "Your Profile has been updated successfully": "Something went wrong!");
      respJson.putIfAbsent("success", () => response.statusCode == 200? true: false);
      //print(responseData);
